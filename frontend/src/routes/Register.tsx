...
function Register() {
  ...
  const [form, setForm] = useState<FormType>({username: '', first_name: '', last_name: '', email: '', password: '', password2: ''})
  const [formError, setFormError] = useState<FormErrorType>({username: null, first_name: null, last_name: null, email: null, password: null, password2: null})
  ...
  useEffect(() =>{
    ...
    setFormError({
      ...
      'first_name': form.username.length > 0 && form.username.length < 2 ? 'Podaj swoje imie' : null,
      'last_name': form.username.length > 0 && form.username.length < 2 ? 'Podaj swoje nazwisko' : null,
